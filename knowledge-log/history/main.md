...
### 追記（2026-07-10）：壁時計が狂った真因の訂正

上で「ブラウザ自動化ごしの壁時計に**拡張機能のオーバーヘッド**が混ざった」と書いたが、これも推測で、正しくない。真因は **自動化中のタブが `hidden` になり、`setTimeout` が1秒に1回へクランプされること**（`計画\04_ポートフォリオ計画.md` §直す順 #8）。`visibilityState: "hidden"` で `setTimeout(12ms)` の実測が `[113, 1000, 1000, 1000, 1000]ms`。`requestAnimationFrame` も hidden では止まる。

つまり「10秒で75字」は**描画の遅さでもオーバーヘッドでもなく、タイマーが動いていなかった**だけ。1描画15〜18ms・long task 0件という計測結果自体は正しい（longtask はタイマーに依存しない）。

**教訓**：ブラウザ自動化の壁時計でフロントの性能を測らない。「遅い」と感じたら、まず `document.visibilityState` と long task を見る。**long task が0なら描画の問題ではない**。原因の推測を2回重ねて、2回とも外した。
